set.seed(423)
r <- 10000
samp <- replicate(r, runif(25, 0, 0.5))
m <- 0
for (i in 1:10000)
{
  m[i] <- max(samp[, i])  
...
no_of_rej/r     
# this value will be close to 0; meaning almost all acceptance.(alpha= 0.05 taken to 
#calculate the critical value, check copy. Under H_0, critical value is unchanged, but the original sample source,
#i.e. the U(0, 0.5) distribution has changed.)
median(samp)
